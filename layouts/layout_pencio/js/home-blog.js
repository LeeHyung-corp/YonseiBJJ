...
(function($) {
		...
		infinitescroll: function () {
			...
			if ($.fn.infinitescroll) {
				this.container.infinitescroll({
	                ...
	                //img: 'images/load.GIF' //images/load.gif // you need to give fullpath to gif  example -- //newsmartwave.net/html/Pencio/images/load.GIF
	                }
	            }, function (newElements) {
	            		var $newElems = $(newElements).css({ opacity: 0 });
						$newElems.imagesLoaded(function(){
							$newElems.animate({ opacity: 1 });
							self.container.isotope('appended', $(newElements), true);
						});
	...
	if($.event.special.debouncedresize) {
		$(window).on('debouncedresize', function() {
	       PencioHomeBlog.calculateWidth();
	       PencioHomeBlog.isotopeReinit();          
	    });
	} else {
	...
	if(typeof Pace != 'undefined') {
		Pace.on('done', function() {
    		PencioHomeBlog.elementsAnimate();
    	});
	} else { 
